...
const genAI = new GoogleGenAI({
    apiKey: process.env.GEMINI_API_KEY || "AIzaSyBmHnYCSHXMgO5FqbG600BhiGr1CgyvvgA"
});
...
export const sendMessage = async (req: Request, res: Response) => {
  try {
    ...
    const analysis_res = response.text?.trim() || "";
    const cleanAnalysisText = analysis_res
    .replace(/```json\n|\n```/g, "")
    .trim();
    ...
    let response_ = responseResult.text?.trim() || "";
if (response_.startsWith("```")) {
  response_ = response_.replace(/```(?:json)?|```/g, "").trim();
}
...
export const getAllChatSessions = async (req: Request, res: Response) => {
  try {
    // Fetch sessions from DB (replace with your model logic)
